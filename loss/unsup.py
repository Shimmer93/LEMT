...
def chamfer_mask(chamfer_dist, x, y, thres_static=0.2, thres_dynamic=0.1):
    # x: B T N C
    # y_hat0: B 1 J 3
    # y_hat1: B 1 J 3

    # x_t0 = x[:, -2:-1, :, :3]  # B 1 N 3
    # x_t1 = x[:, -1:, :, :3]  # B 1 N 3

    dists_x, dists_y = chamfer_dist(x[:, 0].to(torch.float), y[:, 0].to(torch.float))

    mask_dist_pos = (dists_y < thres_dynamic).unsqueeze(1).unsqueeze(-1).detach()  # B 1 J 1
...
class UnsupLoss(torch.nn.Module):
    def __init__(self, thres_static=0.2, thres_dynamic=0.1, thres_loss=0.1):
        super().__init__()
        self.thres_static = thres_static
        self.thres_dynamic = thres_dynamic
    ...
    def forward(self, x, y_hat0, y_hat1):
        ...
        mask_dist_pos, mask_dist_neg = chamfer_mask(self.chamfer_dist, x[:, -1:, :, :3], y_hat1, 
                                                    self.thres_static, self.thres_dynamic)
        ...
        loss_dynamic = torch.relu(self.thres_loss - my_norm_hat_dynamic).mean()
